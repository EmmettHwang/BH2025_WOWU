...
## 📌 버전 정보
- **현재 버전**: v3.105.202601140700
- **최종 업데이트**: 2026년 01월 14일 07시 00분
- **버전 형식**: 메이저.마이너.날짜시간 (예: 3.105.202601140700)

## 🎉 최근 업데이트 (v3.105.202601140700)

### 🔧 최종 수정: DB 연결 정보 확정 (MySQL root 계정)
1. **올바른 DB 연결 정보 (확정)**
   - DB_HOST: localhost
   - DB_PORT: 3306 (MySQL 기본 포트)
   - DB_USER: root (MySQL root 계정)
   - DB_PASSWORD: dodan1004~!@
   - DB_NAME: bh2025

2. **변경 이유**
   - 포트 3307에서 MySQL 실행 안 됨 (Connection refused)
   - iyrc 사용자 접근 거부 (Access denied)
   - MySQL은 포트 3306에서 정상 작동 중
   - root 계정으로 정상 접속 가능
